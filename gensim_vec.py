    ...
    for i in data:  # for each document
        ...
        for j in tokenize(i):  # for each word in the document
            ...
            if j not in both_model:  # word is oov, since both_model gets words from both the negative and positive combined text
                ...
                # print j
                # print w_embed
            elif j not in pos_model and j not in neg_model:  # for some reason, words are in both that are not in pos or neg???
...
if __name__ == "__main__":
    ...
    # modelname='Data/models/out_pos_equal.txt.model'
    # model='Data/models/10milsentences.model'
    # model='Data/models/out_neg.txt.model'
    # model = gensim.models.Word2Vec.load(modelname)
