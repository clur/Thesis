...
def build_embeddings_dict(f,sep='\t'):
    # f='Data/duyu_tang_embedding-results/sswe-h.txt'
    data = [i.strip() for i in open(f).readlines()]
    words = [i.split(sep)[0] for i in data]
    vecs = [i.split(sep)[1:] for i in data]
...
def vectorize_embedding(data, embedding_dict):
    '''
...
def embed_clf_twitter(trainfile, testfile, embedding_dict_file):
    ...
    embedding_dict = build_embeddings_dict(embedding_dict_file, sep=' ')
    # print tr_data[0]
    # vec = tf(ngram_range=(1, 1), max_df=0.9, min_df=0.01, norm=None, use_idf=False)  # basic tfidf vectorizer
    # print 'AVERAGING EMBEDDINGS'
    X_train = vectorize_embedding(tr_data, embedding_dict)
    X_test = vectorize_embedding(te_data, embedding_dict)
    # clf = log()
    # clf=nb()
    # clf=svm()
    for clf in [log, svm]:
...
if __name__ == "__main__":
    ...
    embed_clf_twitter('Data/twitter/twitter.train', 'Data/twitter/twitter.test',
                      '/Users/claire/Desktop/MODELS/pos-50-3.vecs')
