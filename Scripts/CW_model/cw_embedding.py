...
class CW(object):
    ...
    def __init__(self, X, y, y_noise, V, K, B, num_context, n_hidden, unigram):
        '''
    ...
    def concat_and_score(self, X, y):
        ...
        return T.mean(self.score(hiddens))
...
n_hidden = K
words_x = cPickle.load(open('X.pickle','rb'))


words_y = cPickle.load(open('y.pickle','rb'))
...
model = CW(X, y, y_noise, V, K, B, num_context, n_hidden, unigram)
...
train = theano.function([X, y, y_noise],
                        ...
                        updates=updates)
def gen_noise():
    return np.random.randint(low=0, high=V, size=(1, B))
...
for t in xrange(num_batches):
    ...
    y_batch = words_y[start_idx:end_idx][0].astype('int32')
