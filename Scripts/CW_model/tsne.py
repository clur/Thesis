...
if __name__ == "__main__":
    ...
    #    model=gensim.models.Word2Vec.load('/Users/claire/Desktop/MODELS/pos_equal.txt.model')

    # words=['london','paris','france','england','dublin','ireland','germany','berlin']
    data = open('word_embeddings.txt', 'r').readlines()
    data = [d.strip() for d in data]
    labels = [d.split(' ')[0] for d in data]
    X = np.loadtxt('embeddings.txt')
    X = np.array(X)
    X = X[:1000]
    labels = labels[:1000]
